               ...
               int *edge_wgths, int *src_nodes, int K, int *ordering, string f_name, int REFI_OPT, int REFI_ITER)
{
    ...
    int *received_volumes = new int[K];
    ...
    initialize(received_volumes, 0, K, 0);
    ...
    cout << "sum ref : " << sum << endl;
    cout << "sum 2 ref : " << sum2 << endl;
            ...
            for (int local_iter_cnt = 0; local_iter_cnt < (nov / batch_size) + 1; local_iter_cnt++)
            {
                ...
                for (int z = start_point; z < start_point + chunk_size && z < nov; z++)
                {
                    ...
                    if (part_id != decision)
                    {
                        ...
                        if (best_part != -1)
                        {
                            vertices_par[tid][refined_cnt_loc] = vertex;
                            decisions_par[tid][refined_cnt_loc] = decision;
                            refined_cnt_loc++;
                        }
                    }
                }
