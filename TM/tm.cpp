...
//extern void partition_gpu_wrapper(unsigned int* row_ptr, unsigned int* col_ind, unsigned int* net_appearence, int nov, int noe, long long pins, int* edge_wgths, int* src_nodes, int K, int* ordering, const char* gpu_f_name, int REFI_OPT, int REFI_ITER);//GPU

int main(int argc, char *argv[])
{
    CLI::App app("@@@@@@@@@@  HYPERGRAPH PARTITIONER @@@@@@@@@@");
    string f_name, K_CLI = "2", ordering_CLI = "0", refinement_CLI = "0", iter_CLI = "1", num_threads_CLI = "1", gpu_enabled_CLI = "0", gpu_optimized_CLI = "0", gpu_threads_CLI = "0";
    ...
    CLI::Option *opt6 = app.add_option("-g", gpu_enabled_CLI, "use block oriented GPU algorithm, default is 0");
    CLI::Option *opt7 = app.add_option("-w", gpu_optimized_CLI, "use warp oriented GPU algorithm, default is 0");
    CLI::Option *opt8 = app.add_option("-m", gpu_threads_CLI, "use thread oriented GPU algortihm, default is 0");
    ...
    int GPO = atoi(gpu_optimized_CLI.c_str());
    int GPM = atoi(gpu_threads_CLI.c_str());
    ...
    //GRAPH READ //That's a good coding :)
	...
	if(GPE || GPO || GPM){//GPU SWITCH
	  if(GPE == 1 && GPO == 0 && GPM == 0){
	    cout << "#--BLOCK ALGORITHM--#" << endl;
	    partition_gpu(row_ptr, col_ind, net_appearence, nov, noe, pins, edge_wgths, src_nodes, K, ordering, f_name, REFI_OPT, REFI_ITER);//GPU
	  }else if(GPE == 0 && GPO == 1 && GPM == 0){
	    cout << "#--WARP ALGORITHM--#" << endl;
	    partition_gpu_optimized(row_ptr, col_ind, net_appearence, nov, noe, pins, edge_wgths, src_nodes, K, ordering, f_name, REFI_OPT, REFI_ITER);//GPU2
	  }
	  else if(GPE == 0 && GPO == 0 && GPM == 1){
	    cout << "#--THREAD ALGORITHM--#" << endl;
	    partition_gpu_threads(row_ptr, col_ind, net_appearence, nov, noe, pins, edge_wgths, src_nodes, K, ordering, f_name, REFI_OPT, REFI_ITER);//GPU3
	  }
	}
	else{
	  if(NT == 1)
