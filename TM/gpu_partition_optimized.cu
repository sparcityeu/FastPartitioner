...
__global__ void gpu_ref_optimized(int* d_receivers, int* d_senders,
				  ...
				  int* d_part_sizes, int* d_part_messages, int* d_TOTAL_WARPS){
  ...
  int no_arrays = 5; //Number of shared memory array per warp
  ...
  int wid = tid/32;
  int wib = threadIdx.x/32;
  //printf("Total warps: %d, id: %u \n", *d_TOTAL_WARPS, wid);
  //printf("tid: %d, wid: %d, wib: %d \n", tid, wid, wib);
  ...
  int* improvements = &arrays[no_arrays*(wib*K)+(0*K)];
  int* s_ref_marker_receiver = &arrays[no_arrays*(wib*K)+(1*K)];
  int* s_ref_marker_sender = &arrays[no_arrays*(wib*K)+(2*K)];
  int* s_receivers = &arrays[no_arrays*(wib*K)+(3*K)];
  int* s_senders = &arrays[no_arrays*(wib*K)+(4*K)];
  
  for(int cursor = wid; cursor < *d_nov; cursor += *d_TOTAL_WARPS){
    
    //printf("Warp: %d, Cursor: %d, before allocating shareds \n", wid, cursor);    
    ...
    if(tx == 0){
      ...
      //printf("Warp: %d, Cursor: %d, halfway allocating shareds \n", wid, cursor);
    ...
    //printf("Warp: %d, Cursor: %d, after allocating shareds \n", wid, cursor);
    
    unsigned int entry = d_row_ptr[v] + tx;
    unsigned int boundary = d_row_ptr[v+1];
    
    /*
    if(!isShared(improvements))
      printf("WTF! \n");
    
    if(!isShared(s_ref_marker_receiver))
      printf("WTF! \n");

    if(!isShared(s_ref_marker_sender))
      printf("WTF! \n");

    if(!isShared(s_receivers))
      printf("WTF! \n");

    if(!isShared(s_senders))
      printf("WTF! \n");
    */
    
    //if(wid < 5)
    //printf("tid(%d), wid(%d), wib(%d), v(%d), cursor(%d) \n", tid, wid, wib, v, cursor);

    //__syncthreads();
    //return;
    
    //if(tx == 0)
    //printf("Warp(%d), Vertex(%d), entry(%d), boundary(%d) \n", wid, v, entry, boundary);

    //Burada sanki warp id'leri tutturamamam gibi bir sorun var
...
extern void partition_gpu_optimized_wrapper(unsigned int* row_ptr, unsigned int* col_ind, 
  ...
  printf("Shared memory per BLOCK: %d", shared_per_block);
  ...
  //int block_per_sm = 256 * shared_mem_per_multiprocessor / shared_per_block;
  int block_per_sm = 2048/32;
  int thread_per_block = (shared_per_block/(5*K*4))*32;
  if(thread_per_block > 1024)
    thread_per_block = 1024;
  int no_blocks = no_multiprocessors * block_per_sm;
  int total_warps = (no_blocks*thread_per_block)/32;
  int warp_per_block = thread_per_block/32;
  printf("Calculated block per SM: %d\n", block_per_sm);
  printf("Calculated thread_per_block: %d\n", thread_per_block);
  printf("Calculated no of blocks: %d \n", no_blocks);
  printf("Calculated total warps: %d \n", total_warps);
  printf("Calculated warp per block: %d \n", warp_per_block);
  //Device insights for occupancy-----
  ...
  int* d_TOTAL_WARPS;
  cudaMalloc((void**)&d_TOTAL_WARPS, 1*sizeof(int));
  cudaMemcpy(d_TOTAL_WARPS, &total_warps, 1*sizeof(int), cudaMemcpyHostToDevice);
  //DEVICE ALLOCATIONS FOR GLOBAL ARRAYS---------------------------------------------------
  ...
  for(int i = 0; i < REFI_ITER; i++){
    printf("Configuring the kernel with: \n no blocks: %d, thread_per_block: %d, shared memory bytes: %lu\n",no_blocks, thread_per_block, 5*K*warp_per_block*sizeof(int));
    gpu_ref_optimized<<<no_blocks,thread_per_block,5*K*warp_per_block*sizeof(int)>>>(d_receivers,d_senders, d_ref_marker_receiver, d_ref_marker_sender, d_part_vec, d_nov, d_row_ptr, d_col_ind, d_net_appearence, d_ep_starts, d_ep_parts, d_ep_ends, d_src_nodes, d_ep_cnt, d_gain_loss_1d, d_part_connections, d_K, d_best_parts, d_part_sizes, d_part_messages, d_TOTAL_WARPS);
    gpuErrchk2( cudaDeviceSynchronize() );
  }
